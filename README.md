...
```bash
$ make run-cleanup-tests
```
This unconditionally removes NFD, the GPU Operator (namespace, Subscription, OperatorGroup,
CSV, ClusterPolicy), and any leftover gpu-burn resources — see
[Cleaning up leftover resources](#cleaning-up-leftover-resources) below for details.
...
If the GPU operator and gpu burn pod need to be cleaned up, either set `NVIDIAGPU_CLEANUP=true`
in the last execution of either steps 1 or 2, or run `make run-cleanup-tests` (see
...
It can be run standalone via the dedicated `make run-native-dra-tests` target, which sets
`TEST_FEATURES=nvidiagpu` and `TEST_LABELS=native-dra` for you:
```bash
$ export KUBECONFIG=/path/to/kubeconfig
$ export DUMP_FAILED_TESTS=true
$ export REPORTS_DUMP_DIR=/tmp/nvidia-ci-gpu-logs-dir
...
$ export NVIDIAGPU_CLEANUP=false
$ make run-native-dra-tests
```

Equivalently, via the generic `make run-tests` target (e.g. if you want to combine it with
other `TEST_LABELS`):
```bash
$ export TEST_FEATURES="nvidiagpu"
...
[Cleaning up leftover resources](#cleaning-up-leftover-resources) below to remove them
afterwards without re-running the full testcase.
...
without re-running a full deployment, use the dedicated `cleanup` testcase:
...
This runs a lightweight, dedicated Ginkgo testcase (`Label("cleanup")`, part of the `nvidiagpu`
suite) that unconditionally removes NFD, the GPU Operator's Subscription/OperatorGroup/CSV/
Namespace, whichever CR flavor was deployed (`ClusterPolicy` and/or `NVIDIADriver`/
`GPUCluster`), and any leftover gpu-burn pod/configmap/namespace — regardless of the
`NVIDIAGPU_CLEANUP` value. It does not deploy or wait on anything itself, so it completes in
seconds rather than the ~10-15 minutes a full deploy+cleanup cycle would take.

Equivalently: `TEST_FEATURES="nvidiagpu" TEST_LABELS='cleanup' make run-tests`.
...
successfully deleted. Until this is fixed (tracked for a separate PR), it's safest to run
testcases with `NVIDIAGPU_CLEANUP=false` and clean up afterwards with `make run-cleanup-tests`
instead of relying on automatic cleanup.
