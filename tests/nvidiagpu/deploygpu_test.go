...
var _ = Describe("GPU", Ordered, Label(tsparams.LabelSuite), func() {
	...
	Context("DeployGpu", Label("deploy-gpu-with-dtk"), func() {
		...
		It("Deploy NVIDIA GPU Operator with native DRA (NVIDIADriver/GPUCluster)", Label("native-dra"), func() {
			...
			nativeDRANVIDIADriverName = createdNVIDIADriverBuilder.Definition.Name

			By("Wait for NVIDIADriver to be ready")

			err = wait.NVIDIADriverReady(inittools.APIClient, nativeDRANVIDIADriverName,
				nvidiagpu.ClusterPolicyReadyCheckInterval, nvidiagpu.ClusterPolicyReadyTimeout)
			Expect(err).ToNot(HaveOccurred(), "error waiting for NVIDIADriver '%s' to be ready: %v",
				nativeDRANVIDIADriverName, err)
			...
			// Create() is a no-op that returns success without creating anything. Check for
			// pre-existence first so cleanup only ever removes a GPUCluster this It actually
			// created, never one that was already there before this suite ran.
		...
		// This It performs no deployment of its own; it exists purely so that selecting the
		// "cleanup" Ginkgo label causes this Describe's BeforeAll/AfterAll to run without
		// having to re-run (and wait on) a full deploy It first. It is intended to be run on
		// its own (e.g. via `make run-cleanup-tests`) after a previous run left resources in
		// place with NVIDIAGPU_CLEANUP=false, to remove everything unconditionally: NFD, the
		// GPU Operator (ClusterPolicy or NVIDIADriver/GPUCluster, whichever was deployed),
		// its CSV/Subscription/OperatorGroup/Namespace, and any leftover gpu-burn resources.
		It("Cleanup GPU Operator and native DRA resources", Label("cleanup"), func() {
			if !mig.IsLabelInFilter("cleanup") {
				glog.V(gpuparams.GpuLogLevel).Infof("Skipping test: 'cleanup' label not present in ginkgo label filter")
			...
			if err := nfd.Cleanup(inittools.APIClient); err != nil {
				glog.Errorf("Error cleaning up NFD resources: %v", err)
			}

			By("Cleaning up GPU Operator and native DRA resources")
...
func cleanupNativeDRAResources() {
	...
	// Only delete GPUCluster if the "native DRA" It actually created it in this run. GPUCluster
	// is a singleton with a fixed name, so simply finding one by that name doesn't mean this
	// suite owns it: the native DRA It may have been skipped (e.g. no GPU node, or an operator
	// version that doesn't serve the GPUCluster CRD) while a GPUCluster from unrelated setup
	// state still exists. Deleting an unowned GPUCluster here would be a destructive surprise.
	if !nativeDRAGPUClusterOwned {
		glog.V(gpuparams.GpuLogLevel).Infof(
			"GPUCluster was not created by this test run, skipping deletion")
	} else if gpuClusterBuilder, err := nvidiagpu.PullGPUCluster(inittools.APIClient, nvidiagpu.GPUClusterName); err == nil {
		if err := gpuClusterBuilder.DeleteAndWait(nativeDRADeleteTimeout); err != nil {
	...
	if nativeDRANVIDIADriverName != "" {
		...
		if nvidiaDriverBuilder, err := nvidiagpu.PullNVIDIADriver(inittools.APIClient, nativeDRANVIDIADriverName); err == nil {
			if err := nvidiaDriverBuilder.DeleteAndWait(nativeDRADeleteTimeout); err != nil {
				glog.Errorf("Error deleting NVIDIADriver %s: %v", nativeDRANVIDIADriverName, err)
			} else {
	...
	// Standalone cleanup run in a separate process (e.g. `make run-cleanup-tests`):
	...
	if err != nil {
		glog.Errorf("Error listing NVIDIADriver objects owned by this testcase: %v", err)
	...
	for _, nvidiaDriver := range nvidiaDrivers {
		...
		if err := nvidiaDriverBuilder.DeleteAndWait(nativeDRADeleteTimeout); err != nil {
			glog.Errorf("Error deleting NVIDIADriver %s: %v", nvidiaDriver.Name, err)
		} else {
